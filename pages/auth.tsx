    ...
    <div className="relative h-full w-full bg-[url('/images/hero.jpg')] bg-no-repeat bg-center bg-fixed bg-cover">
        <div className="w-full h-full lg:bg-opacity-50 bg-black">
            <nav className="px-12 py-5">
               <img src="/images/download.jpeg" alt="logo" className="h-14"/>
            </nav>
            <div className="flex justify-center">
                <div className="bg-black bg-opacity-80 px-16 py-16 self-center mt-1 lg:w-2/5 lg:max-w-md rounded-md w-full">
                    <h2 className="text-4xl mb-6 text-white font-semibold">{variant === 'login'?'Sign in':'Register'}</h2>
                    <div className="flex flex-col gap-4">
                    {variant==='register' && (<Input id="name"
                label="Username"
                value={name}
                ...
                onChange={(e: any) => setPassword(e.target.value)}   />
                    ...
                    <button onClick={variant==='login'?login:register} className="bg-red-600 py-3 text-white rounded-md w-full mt-6 hover:bg-red-700 transition">{variant==='login'?'Login':'Sign Up'}</button>
              ...
              <div onClick={() => signIn('github', { callbackUrl: '/profile' })}  className="w-10 h-10 bg-white rounded-full flex items-center justify-center cursor-pointer hover:opacity-80 transition">
                <FaGithub size={32} />
                    ...
                    <p className="text-neutral-500 mt-3">
                        {variant==='login'?'First time using MoviesHub?':'Already have an account '}
